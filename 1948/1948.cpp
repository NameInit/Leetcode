...
int main(){
	Solution test;
	{
		std::vector<std::vector<std::string>> paths={{"a"},{"c"},{"d"},{"a","b"},{"c","b"},{"d","a"}};
		test.deleteDuplicateFolder(paths);
	}
	{
		std::vector<std::vector<std::string>> paths={{"a"},{"c"},{"a","b"},{"c","b"},{"a","b","x"},{"a","b","x","y"},{"w"},{"w","y"}};
		test.deleteDuplicateFolder(paths);
	}
	{
		std::vector<std::vector<std::string>> paths={{"a","b"},{"c","d"},{"c"},{"a"}};
		test.deleteDuplicateFolder(paths);
	}
	{
		std::vector<std::vector<std::string>> paths={{"a"},{"b"},{"a","c"},{"b","c"},{"a","c","d"},{"b","c","d"}};
		test.deleteDuplicateFolder(paths);
	}
	return 0;
}